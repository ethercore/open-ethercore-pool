...
import (
	...
	"github.com/ethercoreorg/open-ethercore-pool/payouts"
	"github.com/ethercoreorg/open-ethercore-pool/rpc"
	"github.com/ethercoreorg/open-ethercore-pool/storage"
	"github.com/ethercoreorg/open-ethercore-pool/util"
)
...
func (s *ApiServer) StatsIndex(w http.ResponseWriter, r *http.Request) {
	...
	if nodes[0] != nil {
		...
		era := payouts.GetBlockEra(big.NewInt(number), big.NewInt(5000000))
		reply["blockReward"] = payouts.GetBlockWinnerRewardByEra(era).String()
	}
